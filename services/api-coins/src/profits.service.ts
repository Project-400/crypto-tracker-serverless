...
import { CoinCount, PairPriceList } from '../../api-valuation/src/valuation.service';
...
export class ProfitsService {
	...
	public getInvestmentChange = async (coin: string): Promise<any> => {
		...
		console.log('COIN USD: ', coinUsdPrice);
		console.log('COIN BNB: ', coinBnbPrice);
		console.log('BNB: ', BNBBUSD_Price);
		...
		const currentBtcPrice: number = Number((coinUsdtPrice / BTCUSDT_Price));
		...
		const details: any = {
			...
			currentPrice: Number(this.BTCtoUSDT(BTCUSDT_Price, currentBtcPrice).toFixed(7)),
