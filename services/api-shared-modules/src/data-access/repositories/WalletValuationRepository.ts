...
export class WalletValuationRepository extends Repository implements IWalletValuationRepository {
	...
	public async get(userId: string, interval: VALUE_LOG_INTERVAL, roundedTime: string): Promise<WalletValue> {
		try {
	...
	public async create(userId: string, walletValue: Partial<WalletValue>): Promise<WalletValue> {
		const date: string = new Date().toISOString();

		return this.db.put(Object.assign(new WalletValuationItem(), {
			pk: `${Entity.WALLET_VALUATION}#${walletValue.time}`,
			sk: `${Entity.USER}#${userId}/${EntitySortType.TIME}#${walletValue.time}`,
	...
	// public async update(roundedHour: string, userId: string, changes: Partial<WalletValue>):
	// 	Promise<WalletValue> {
	// 	delete changes.sk2;
	// 	delete changes.sk3;
	//
